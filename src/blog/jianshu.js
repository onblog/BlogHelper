 ...
 * 上传文章到简书
 */
exports.publishArticleToJianshu = function publishArticleToJianShu(title, content) {
    return new Promise((resolve, reject) => {
        // 获取作品集
        https.get('https://www.jianshu.com/author/notebooks',
                  ...
                  (response) => {
                      ...
                      response.on('end', () => {
                          ...
                          if (response.statusCode === 200) {
                              const parms = JSON.parse(body);
                              createNote(resolve, reject, parms, title, content)
                          } else {
...
function createNote(resolve, reject, parms, title, content) {
    ...
    let request = https.request({
        ...
        res.on('end', () => {
            ...
            if (res.statusCode === 200) {
                ...
                console.log(result)
                if (result.id) {
                    updateNote(resolve, reject, notebookId, result.id, title, content)
                } else {
                    reject('发布失败,' + result)
                }
            } else {
...
function updateNote(resolve, reject, notebookId, id, title, content) {
    const json = JSON.stringify({
                                    id: id,
                                    title: title,
                                    content: content,
